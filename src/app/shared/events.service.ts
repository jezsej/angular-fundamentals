...
export class EventService {
  ...
  getEvents(): Observable<IEvent[]> {
    return this.http.get<IEvent[]>('/api/events')
      .pipe(catchError(this.handleErrors<IEvent[]>('getEvents', [])))
  }

  saveEvent(event) {
    let options = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) }
    return this.http.post('/api/events', event, options)
      .pipe(catchError(this.handleErrors<IEvent>('saveEvent')))
  }
  ...
  searchSession(term: string): Observable<ISession[]> {
    return this.http.get<ISession[]>(`/api/sessions/search?search=${term}`)
      .pipe(catchError(this.handleErrors<ISession[]>('searchSession', [])))
    /*let searchterm = term.toLocaleLowerCase()
    let sessions: ISession[] = []

    EVENTS.forEach(event => {
      let matchingSession = event.sessions.filter(s => {
        return s.name.toLocaleLowerCase().indexOf(searchterm) > -1
      })
      ...
      sessions = sessions.concat(matchingSession)
    })
    let eventEmitter = new EventEmitter(true)

    setTimeout(() => {
      eventEmitter.emit(sessions)
    }, 100)
    return eventEmitter*/
  }
